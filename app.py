...
channel_secret = 'e0b2df648caa638498d0c67962a45de4'
channel_access_token = 'cWjkqtO8UU3VAaoWf5loqLiX9gDaT+e0goJigJJ0pgnET7H74aKwYN3/4juvDZb1KsBIjW5wkMKmQOqKeXD3md2gxvHwbULQgBnm3VMIdTokt7stObu6AvCITwGS1AKhefoexEd09o0/uLuGd4u3oAdB04t89/1O/w1cDnyilFU='
...
def handle_text_message(event):
    ...
    textRef = []
    url = 'https://dev-scglmw.scglogistics.co.th/api/1/rest/feed-master/queue/SCG/CBM_SCGL/SCG_Logistics_Chat_Bot_Tranfer_Coal/COALCHATBOT_GET_VESSELNAME'
    xxxx= {
                "Content-Type": "application/json",
                "Authorization": "Bearer VpO8oHZ1P1arpeAQ0RrP9JxBYmg98lkn",
                "Content-Type":"application/json"
        }
    req = requests.post(url, headers=xxxx)
    print(req.content)
    req_con = json.loads(req.content)
    ms_text = ''
    for z in req_con:
        textRef.append(z["VESSELNAME"])

    if text in textRef:
            url_status = 'https://dev-scglmw.scglogistics.co.th/api/1/rest/feed-master/queue/SCG/CBM_SCGL/SCG_Logistics_Chat_Bot_Tranfer_Coal/COALCHATBOT_STATUSDATA'
            header_status = {
                                "Content-Type": "application/json",
                                "Authorization": "Bearer VpO8oHZ1P1arpeAQ0RrP9JxBYmg98lkn",
                                "Content-Type":"application/json"
            ...
            req_status = requests.post(url_status, data=json.dumps(payload), headers=header_status)
            print(req_status.content)

            req_status = json.loads(req_status.content)

            matName = req_status["MATERIALNAME"]
            csGroup = req_status["CS_Group"]
            booking = req_status["BOOKINGQUEUE"]
            callQueue = req_status["CALLQUEUE"]

            # %S 1,Coal name 2, Name VESSELNAME 3,group tf 4, time
            bubble_string = """{
                                "type": "bubble",
                                "direction": "ltr",
                                "header": {
                            ...
                            """ % (matName,text,csGroup,"2020-08-01")
        ...
        line_bot_api.reply_message(event.reply_token, messages)
            ...
            ms_text = ''
            for z in y:
                ms_text += z["VESSELNAME"] + '\n'

            line_bot_api.reply_message(
                event.reply_token, [
                    TextSendMessage(text='Test Send Message Reply \n' + ms_text ),
                ]
            )
    else:
        line_bot_api.reply_message(
